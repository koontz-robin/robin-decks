...
def fetch():
    ...
    AND (Account_Tiers__c = null OR Account_Tiers__c NOT IN ('International','Bad','Fringe'))
    AND Type NOT IN ('Rev.io PSA Client','TigerPaw Client','Rev.io Billing Client')
    ORDER BY BillingStateCode,NumberOfEmployees DESC,Name"""
    rows=[]
    for r in sf_query_all(base,headers,q):
        owner=r.get('Owner') or {}
        if c(r.get('Account_Tiers__c')).lower() in {'international','bad','fringe'}:
            continue
        if c(r.get('Type')).lower() in {'rev.io psa client','tigerpaw client','rev.io billing client'}:
            continue
...
<body><main class="wrap"><header><div><h1>Accounts with 35+ Employees</h1><p class="lede">All Salesforce accounts with <b>35 or more employees</b> in RI, MA, CT, NJ, NY, FL, DE, GA, AL, MS, DC, SC, NC, TN, MD, VA, WV, KY, PA, OH, IN, NH, VT, MI, and ME. No owner or activity exclusions applied; International, Bad, and Fringe tiers plus Rev.io PSA, TigerPaw, and Billing client account types are excluded.</p></div><div class="stamp">Generated {e(generated)}<br>Source: Salesforce Account object</div></header>
