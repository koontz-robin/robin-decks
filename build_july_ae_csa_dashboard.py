#!/usr/bin/env python3
"""Build and publish the July AE/CSA opportunity creation dashboard."""
...
HTML_FILE = WORKSPACE / "july-ae-csa-opportunities.html"
DATA_FILE = WORKSPACE / "sf_july_ae_csa_opps.json"
...
AUGUST_START = "2026-08-01T04:00:00Z"
TEAM_TARGET = 275
PRODUCT_GOALS = [
    ("PSA", 200),
    ("Billing/Odin", 40),
...
WEEKS = [
    ("Week 1", "Jul 1-7", "2026-07-01", "2026-07-07"),
    ("Week 2", "Jul 8-14", "2026-07-08", "2026-07-14"),
    ("Week 3", "Jul 15-21", "2026-07-15", "2026-07-21"),
    ("Week 4", "Jul 22-28", "2026-07-22", "2026-07-28"),
    ("Week 5", "Jul 29-31", "2026-07-29", "2026-07-31"),
]
...
def fetch_opportunities(base, headers):
    query = f"""
        ...
        FROM Opportunity
        WHERE CreatedDate >= {JULY_START}
          AND CreatedDate < {AUGUST_START}
          AND IsDeleted = false
            ...
            <tr class="details-row">
              <td colspan="7">
                <div class="opp-detail-panel">
                  ...
                  {opp_detail_rows(opps_by_rep.get(rep, []), 'No opportunities created in July.')}
                </div>
              </td>
            </tr>"""
        )
            ...
            <tr class="details-row">
              <td colspan="7">
                <div class="opp-detail-panel sdr">
                  ...
                  {opp_detail_rows(sdr_opps_by_rep.get(sdr, []), 'No SDR-influenced opportunities created in July.')}
                </div>
              </td>
            </tr>"""
        )
    if not rows:
        return '<tr><td colspan="7" class="empty-table">No SDR-influenced opportunities created in July.</td></tr>'
    return "\n".join(rows)
...
<title>July Opportunity Created Report</title>
<style>
:root {{
  --bg:#f6f8fb;
  --panel:#ffffff;
...
<main class="shell">
  <header class="topbar">
    <div>
      <div class="eyebrow">July 2026 Pipeline Creation</div>
      <h1>July Opportunity Created Report</h1>
      <div class="subtitle">Quantity and MRR by owner, split across all five July calendar weeks. $0 opportunities excluded.</div>
    </div>
    <div class="stamp">
  ...
  <section class="kpis">
    <div class="kpi target-card">
      ...
      <div class="target-meta"><span>{remaining} opps remaining</span><span>July target</span></div>
    </div>
  ...
  <section class="section-title">
    <div>
      ...
      <p>July-created opportunities with SDR Influence populated, grouped by SDR.</p>
    </div>
...
def publish(files):
    ...
    tmp_parent = Path(tempfile.mkdtemp(prefix="july-ae-csa-dashboard-"))
    ...
    try:
        ...
        subprocess.run(["git", "commit", "-m", f"add July AE CSA opportunity dashboard ({stamp})"], cwd=worktree, check=True)
...
def main():
    ...
    print("Fetching July-created opportunities...")
