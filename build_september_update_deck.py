...
'''
    ...
    billing_interest_raw=sf_query(base,headers,"SELECT Id,Name,CreatedDate,CloseDate,StageName,Account.Id,Account.Name FROM Opportunity WHERE CreatedDate>=2025-07-01T00:00:00Z AND Product_Type__c IN ('PSA','PSA 2.0') AND Account.Type='Rev.io Billing Client' ORDER BY CreatedDate DESC")
    billing_interest={}
    ...
    billing_clients=sorted(billing_interest.values(),key=lambda x:((x.get('CloseDate') or '9999-12-31'),((x.get('Account') or {}).get('Name') or '').lower()))
    ...
    for x in billing_clients:
        ...
        rows.append(f'<div class="billing-interest-row"><div class="billing-client" title="{escape(account.get("Name") or "Unknown")}">{escape(account.get("Name") or "Unknown")}</div><div class="billing-date">{escape(x.get("CloseDate") or "No date")}</div><div class="billing-stage{cls}">{escape(stage_short.get(stage,stage))}</div></div>')
    chunk=(len(rows)+2)//3
