...
'''
    ...
    slides.append(f'<div class="slide" id="slide-3">{header("Current Month Snapshot","September Pipeline Snapshot")}<div class="slide-body"><div class="kpi-row" style="grid-template-columns:repeat(3,1fr)">{card("Deals Won MTD",len(sep_won),"Closed-won count")}{card("Closed Won MRR",money(amount(sep_won)),"September MTD result","green")}{card("Open Pipeline",len(sep_open),f"opps · {money(amount(sep_open))} value","navy")}</div>{section("September Product Results — Quota Attainment")}{product_cards(sep_won,sep_open,quotas)}</div></div>')
    eff_cards=[]
    for m in range(1,10):
