...
'''
    ...
    slides.append(f'<div class="slide" id="slide-tigerpaw">{header("Legacy Migration Opportunity Type","Tigerpaw Migration Progress — Last Three Months")}<div class="slide-body">{section("Monthly Migration Progress")}<div class="migration-grid">{"".join(migration_cards)}</div>{section("Current Active Migration Funnel")}<div class="stage-flow">{stage_html}</div>{section("July–September Progress Summary")}<div class="summary-strip"><div><strong>{migration_totals["created"]}</strong><span>Migration opportunities created</span></div><div><strong>{migration_totals["won"]}</strong><span>Closed Won migrations</span></div><div><strong>{overall_rate:.0f}%</strong><span>Decision win rate</span></div></div></div></div>')
    ...
    def win_rows(rows):
        ...
        for x in rows:
            ...
            renewal=x.get('Renewal_Amount__c'); uplift=x.get('Amount')
            out.append(f'<tr><td class="client">{escape(client)}</td><td>{escape(x.get("CloseDate") or "")}</td><td class="money">{money(renewal) if renewal is not None else "—"}</td><td class="uplift">{money(uplift) if uplift is not None else "—"}</td></tr>')
        return ''.join(out)
    midpoint=(len(legacy_wins)+1)//2
    def wins_table(rows):
        return f'<div class="data-table wins-table"><table><thead><tr><th>Client</th><th>Won</th><th>Renewal</th><th>Uplift (Amount)</th></tr></thead><tbody>{win_rows(rows)}</tbody></table></div>'
    renewal_total=sum(float(x.get('Renewal_Amount__c') or 0) for x in legacy_wins)
    uplift_total=sum(float(x.get('Amount') or 0) for x in legacy_wins)
    slides.append(f'<div class="slide" id="slide-tigerpaw-wins">{header("Legacy Migration Closed Won","Tigerpaw Clients Won — Renewal and Uplift")}<div class="slide-body">{section("July–September Closed Won Clients")}<div class="wins-columns">{wins_table(legacy_wins[:midpoint])}{wins_table(legacy_wins[midpoint:])}</div><div class="summary-strip"><div><strong>{len(legacy_wins)}</strong><span>Clients won</span></div><div><strong>{money(renewal_total)}</strong><span>Total renewal amount</span></div><div><strong>{money(uplift_total)}</strong><span>Total uplift from Amount field</span></div></div></div></div>')
    billing_interest_raw=sf_query(base,headers,"SELECT Id,Name,CreatedDate,StageName,Account.Id,Account.Name FROM Opportunity WHERE CreatedDate>=2025-07-01T00:00:00Z AND Product_Type__c IN ('PSA','PSA 2.0') AND Account.Type='Rev.io Billing Client' ORDER BY CreatedDate DESC")
    billing_interest={}
    ...
    billing_clients=sorted(billing_interest.values(),key=lambda x:((x.get('Account') or {}).get('Name') or '').lower())
    ...
    for x in billing_clients:
        account=x.get('Account') or {}; stage=x.get('StageName') or 'Other'; cls=' lost' if stage=='Closed Lost' else ''
        rows.append(f'<div class="billing-interest-row"><div class="billing-client" title="{escape(account.get("Name") or "Unknown")}">{escape(account.get("Name") or "Unknown")}</div><div class="billing-date">{escape((x.get("CreatedDate") or "")[:10])}</div><div class="billing-stage{cls}">{escape(stage_short.get(stage,stage))}</div></div>')
    chunk=(len(rows)+2)//3
    ...
    slides.append(f'<div class="slide" id="slide-billing-interest">{header("Billing Client Expansion","Billing Clients Showing Interest in New Rev.io Since July 2025")}<div class="slide-body" style="padding:12px 28px 56px"><div class="summary-strip"><div><strong>{len(billing_clients)}</strong><span>Billing clients with New Rev.io opportunity</span></div><div><strong>{open_interest}</strong><span>Currently open</span></div><div><strong>{won_interest} / {lost_interest}</strong><span>Won / Lost</span></div></div>{section("Most Recent New Rev.io Opportunity by Billing Client")}<div class="billing-interest-grid">{columns}</div></div></div>')
    ...
    ids=['slide-1','slide-5','slide-tigerpaw','slide-tigerpaw-wins','slide-billing-interest','slide-6']; labels=['August + September','Pipeline Trends','Tigerpaw Migration','Tigerpaw Wins','Billing Client Interest','Closed Lost']
