...
'''
    ...
    slides.append(f'<div class="slide" id="slide-tigerpaw-wins">{header("Legacy Migration Closed Won","Tigerpaw Clients Won — Renewal Amounts")}<div class="slide-body">{section("July–September Closed Won Clients")}<div class="wins-columns">{wins_table(legacy_wins[:midpoint])}{wins_table(legacy_wins[midpoint:])}</div><div class="summary-strip" style="grid-template-columns:repeat(2,1fr)"><div><strong>{len(legacy_wins)}</strong><span>Clients won</span></div><div><strong>{money(renewal_total)}</strong><span>Total renewal amount</span></div></div></div></div>')
