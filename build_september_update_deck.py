...
'''
    slides=[]
    slides.append(f'<div class="slide active" id="slide-1">{header("Prior Month + Current Month Snapshot","August Final and September MTD")}<div class="slide-body"><div class="kpi-row april-final-row">{card("August Deals Won",len(aug_won),"Final August count")}{card("August Closed Won MRR",money(amount(aug_won)),"Final August result","green")}</div>{section("August Product Results — Quota Attainment")}{product_cards(aug_won,[],quotas,False)}{section("September Product Results — Quota Attainment")}{product_cards(sep_won,sep_open,quotas)}</div></div>')
