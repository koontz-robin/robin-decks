"""
patch_may_forecast.py — Patches May tab + Q2 bar in forecast.html with fresh SF data.
Keeps all other tabs, formatting, and structure intact.
"""
import json, re
...
with open(f'{WORKSPACE}/sf_may_opps.json') as f:
    opps = json.load(f)
...
MAY_QUOTAS = {'PSA':36000,'Billing':10252,'Payments':11040,'Cyber':2478,'CommerceHub':2956}
...
def build_may_tab():
    lines = ['<div id="tab-May" class="tab-content" style="display:block">']
    ...
    lines.append(f'''    <div class="summary-bar">
      ...
      <div class="sum-item"><div class="sum-label">May CW</div><div class="sum-val">{fmt(total_cw)}</div></div>
    ...
    for idx, p in enumerate(PRODUCTS):
        ...
        lines.append(f'''
<div class="product-section" id="prod-may-{idx}">
  ...
  <div class="quota-bar-wrap">
    <div class="quota-bar-header">
      <span class="quota-bar-label">MAY QUOTA ATTAINMENT</span>
  ...
  <div class="opp-toggle" onclick="toggle('may-{idx}')">
    <span id="toggle-label-may-{idx}">▶ Show all {len(opp_list)} opportunities</span>
    <span class="toggle-amt">{fmt(pipe)} total</span>
  </div>
  <div id="opps-may-{idx}" class="opp-list" style="display:none">
...
def build_q2_bar():
    q2_cw = {p: apr_cw.get(p,0) + buckets[p]['closed'] for p in PRODUCTS}
...
# Replace May tab
may_start = html.index('<div id="tab-May"')
may_end   = html.index('<div id="tab-June"')
html = html[:may_start] + build_may_tab() + '\n  ' + html[may_end:]
...
total_cw  = sum(buckets[p]['closed'] for p in PRODUCTS)
total_pipe = sum(o.get('Amount',0) or 0 for o in opps if o.get('StageName') != 'Closed Won')
print(f'May data patched — CW: {fmt(total_cw)} | Pipeline: {fmt(total_pipe)} | {sum(len(buckets[p]["opps"]) for p in PRODUCTS)} open opps')
